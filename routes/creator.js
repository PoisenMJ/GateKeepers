...
router.get('/products/own/:creatorTag', (req, res) => {
  ...
  creator.findOne({ tag: req.params.creatorTag }).then((c, e) => {
    ...
    if (c) {
      creatorProduct.find({ type: 'own' }).populate({
        ...
        if (err) return res.status(400).json(err);
        const products = docs.filter((a) => a.creator);
...
router.get('/products/made/:creatorTag', (req, res) => {
  // eslint-disable-next-line consistent-return
  creator.findOne({ tag: req.params.creatorTag }).then((c, e) => {
    if (e) return res.json({ success: false });
    if (c) {
...
router.get('/products/:productURI/:type', (req, res) => {
...
router.get('/all-creators', (req, res) => {
...
router.get('/shipping/:creatorTag', (req, res) => {
...
router.get('/accent/:creatorTag', (req, res) => {
