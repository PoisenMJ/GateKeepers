...
const createSocket = () => io(SOCKET_HOST);
const joinRoom = (_socket, user1, user2) => {
  console.log(`Client joining: ${user1}-${user2}`);
  _socket.emit('join_room', user1, user2);
};
const joinGeneralRoom = (_socket, _user) => {
  console.log(`Joining general room: ${_user}`);
  _socket.emit('join_creator_room', _user);
};
const sendMessage = (_socket, user1, user2, _message, _type = 'message') => {
  _socket.emit('customs_message', user1, user2, _message, _type);
};
const onMessageRecieved = (_socket, _callback) => {
  _socket.on('customs_message', (user1, user2, _message, _type) => {
    _callback(user1, user2, _message, _type);
  });
};
